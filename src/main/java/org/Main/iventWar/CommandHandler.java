...
public class CommandHandler implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (cmd.equals("topbro")) { handleTopBro(player); return true; }
        if (cmd.equals("tc")) { handleTeamChat(player, args); return true; }
        if (cmd.equals("myteam") || (cmd.equals("my") && args.length > 0 && args[0].equalsIgnoreCase("team"))) {
            handleMyTeam(player); return true;
        }
        ...
        if (cmd.equals("closeevent")) {
            if (!player.isOp()) { player.sendMessage(ChatColor.RED + "Нет прав!"); return true; }
            if (args.length == 0) { player.sendMessage(ChatColor.RED + "Использование: /closeEvent <причина>"); return true; }
            StringBuilder reason = new StringBuilder();
            for (String arg : args) reason.append(arg).append(" ");
            String reasonText = reason.toString().trim();
            if (!plugin.getEventManager().isEventActive()) { player.sendMessage(ChatColor.RED + "Ивент не активен!"); return true; }
    ...
    private void handleTeamChat(Player player, String[] args) {
        if (args.length == 0) { player.sendMessage(ChatColor.RED + "Использование: /tc <сообщение>"); return; }
    ...
    private void handleMyTeam(Player player) {
        Team team = teamManager.getPlayerTeam(player.getUniqueId());
        if (team == null) { player.sendMessage(ChatColor.RED + "К сожалению, ты не состоишь в команде!"); return; }
        ...
        for (UUID member : team.getMembers()) {
            ...
            String roleDisplay = role.equals("leader") ? ChatColor.GOLD + "★ Лидер" :
                    role.equals("helper") ? ChatColor.AQUA + "✪ Помощник" : ChatColor.GRAY + "• Участник";
            player.sendMessage(ChatColor.GOLD + "║   " + roleDisplay + ChatColor.WHITE + ": " + name + " " + prefix);
        }
        player.sendMessage(ChatColor.GOLD + "╚═══════════════════════════════════╝");
        if (team.isLeader(player.getUniqueId())) player.sendMessage(ChatColor.GREEN + "Ты лидер!");
    ...
    private void handleCreate(Player player, String[] args) {
        ...
        if (teamManager.getTeam(teamName) != null) { player.sendMessage(ChatColor.RED + "Команда уже существует!"); return; }
        if (teamManager.getPlayerTeam(player.getUniqueId()) != null) { player.sendMessage(ChatColor.RED + "Ты уже в команде!"); return; }
    ...
    private void handleInvite(Player player, String[] args) {
        if (args.length < 2) { player.sendMessage(ChatColor.RED + "Использование: /team invite <игрок>"); return; }
        Team team = teamManager.getPlayerTeam(player.getUniqueId());
        if (team == null) { player.sendMessage(ChatColor.RED + "Ты не в команде!"); return; }
        if (!team.isLeader(player.getUniqueId()) && !team.isHelper(player.getUniqueId())) {
            player.sendMessage(ChatColor.RED + "Только лидер или помощник могут приглашать!"); return;
        }
        Player target = Bukkit.getPlayer(args[1]);
        if (target == null) { player.sendMessage(ChatColor.RED + "Игрок не найден!"); return; }
        if (teamManager.getPlayerTeam(target.getUniqueId()) != null) { player.sendMessage(ChatColor.RED + "Игрок уже в команде!"); return; }
    ...
    private void handleAccept(Player player) {
        ...
        if (teamName == null) { player.sendMessage(ChatColor.RED + "У тебя нет приглашений!"); return; }
        Team team = teamManager.getTeam(teamName);
        if (team == null) { player.sendMessage(ChatColor.RED + "Команда не существует!"); teamManager.removeInvitation(player.getUniqueId()); return; }
        if (teamManager.addPlayerToTeam(teamName, player.getUniqueId())) {
            ...
            for (UUID member : team.getMembers()) {
                Player p = Bukkit.getPlayer(member);
                if (p != null && !p.equals(player)) p.sendMessage(ChatColor.YELLOW + player.getName() + " присоединился!");
            }
            teamManager.removeInvitation(player.getUniqueId());
            teamManager.updateAllTeamTab(team);
        } else player.sendMessage(ChatColor.RED + "Не удалось присоединиться!");
    }
    ...
    private void handleKick(Player player, String[] args) {
        if (args.length < 2) { player.sendMessage(ChatColor.RED + "Использование: /team kick <игрок>"); return; }
        Team team = teamManager.getPlayerTeam(player.getUniqueId());
        if (team == null) { player.sendMessage(ChatColor.RED + "Ты не в команде!"); return; }
        if (!team.isLeader(player.getUniqueId())) { player.sendMessage(ChatColor.RED + "Только лидер может исключать!"); return; }
        Player target = Bukkit.getPlayer(args[1]);
        if (target == null) { player.sendMessage(ChatColor.RED + "Игрок не найден!"); return; }
        if (target.equals(player)) { player.sendMessage(ChatColor.RED + "Нельзя исключить себя! Используй /team leave."); return; }
        if (!team.isMember(target.getUniqueId())) { player.sendMessage(ChatColor.RED + "Игрок не в твоей команде!"); return; }
    ...
    private void handleDesc(Player player, String[] args) {
        ...
        if (team == null) { player.sendMessage(ChatColor.RED + "Ты не в команде!"); return; }
        if (!team.isLeader(player.getUniqueId()) && !team.isHelper(player.getUniqueId())) {
            player.sendMessage(ChatColor.RED + "Только лидер или помощник могут менять описание!"); return;
        }
        if (args.length < 2) { player.sendMessage(ChatColor.RED + "Использование: /team desc <описание>"); return; }
    ...
    private void handleColor(Player player, String[] args) {
        if (args.length < 2) { player.sendMessage(ChatColor.RED + "Использование: /team color <цвет>"); return; }
        Team team = teamManager.getPlayerTeam(player.getUniqueId());
        if (team == null) { player.sendMessage(ChatColor.RED + "Ты не в команде!"); return; }
        if (!team.isLeader(player.getUniqueId()) && !team.isHelper(player.getUniqueId())) {
            player.sendMessage(ChatColor.RED + "Только лидер или помощник могут менять цвет!"); return;
        }
        try {
    ...
    private void handlePrefix(Player player, String[] args) {
        if (args.length < 2) { player.sendMessage(ChatColor.RED + "Использование: /team prefix <позывной>"); return; }
        Team team = teamManager.getPlayerTeam(player.getUniqueId());
        if (team == null) { player.sendMessage(ChatColor.RED + "Ты не в команде!"); return; }
    ...
    private void handleTransfer(Player player, String[] args) {
        if (args.length < 2) { player.sendMessage(ChatColor.RED + "Использование: /team transfer <игрок>"); return; }
        Team team = teamManager.getPlayerTeam(player.getUniqueId());
        if (team == null) { player.sendMessage(ChatColor.RED + "Ты не в команде!"); return; }
        if (!team.isLeader(player.getUniqueId())) { player.sendMessage(ChatColor.RED + "Только лидер может передавать лидерство!"); return; }
        Player target = Bukkit.getPlayer(args[1]);
        if (target == null) { player.sendMessage(ChatColor.RED + "Игрок не найден!"); return; }
        if (target.getUniqueId().equals(player.getUniqueId())) { player.sendMessage(ChatColor.RED + "Нельзя передать лидерство самому себе!"); return; }
        if (!team.isMember(target.getUniqueId())) { player.sendMessage(ChatColor.RED + "Игрок не в твоей команде!"); return; }
        ...
        } else player.sendMessage(ChatColor.RED + "Не удалось передать лидерство.");
    }

    private void handlePromote(Player player, String[] args) {
        if (args.length < 2) { player.sendMessage(ChatColor.RED + "Использование: /team promote <игрок>"); return; }
        Team team = teamManager.getPlayerTeam(player.getUniqueId());
        if (team == null) { player.sendMessage(ChatColor.RED + "Ты не в команде!"); return; }
        if (!team.isLeader(player.getUniqueId())) { player.sendMessage(ChatColor.RED + "Только лидер может назначать помощников!"); return; }
        Player target = Bukkit.getPlayer(args[1]);
        if (target == null) { player.sendMessage(ChatColor.RED + "Игрок не найден!"); return; }
        if (target.getUniqueId().equals(player.getUniqueId())) { player.sendMessage(ChatColor.RED + "Нельзя сделать себя помощником!"); return; }
        if (!team.isMember(target.getUniqueId())) { player.sendMessage(ChatColor.RED + "Игрок не в твоей команде!"); return; }
        if (team.isLeader(target.getUniqueId())) { player.sendMessage(ChatColor.RED + "Лидер не может быть помощником!"); return; }
        ...
        } else player.sendMessage(ChatColor.RED + "Не удалось назначить помощника.");
    }

    private void handleDemote(Player player, String[] args) {
        if (args.length < 2) { player.sendMessage(ChatColor.RED + "Использование: /team demote <игрок>"); return; }
        Team team = teamManager.getPlayerTeam(player.getUniqueId());
        if (team == null) { player.sendMessage(ChatColor.RED + "Ты не в команде!"); return; }
        if (!team.isLeader(player.getUniqueId())) { player.sendMessage(ChatColor.RED + "Только лидер может понижать!"); return; }
        Player target = Bukkit.getPlayer(args[1]);
        if (target == null) { player.sendMessage(ChatColor.RED + "Игрок не найден!"); return; }
        if (target.getUniqueId().equals(player.getUniqueId())) { player.sendMessage(ChatColor.RED + "Нельзя понизить себя!"); return; }
        if (!team.isMember(target.getUniqueId())) { player.sendMessage(ChatColor.RED + "Игрок не в твоей команде!"); return; }
        if (team.isLeader(target.getUniqueId())) { player.sendMessage(ChatColor.RED + "Лидера нельзя понизить!"); return; }
        ...
        } else player.sendMessage(ChatColor.RED + "Не удалось понизить.");
    }
